...
class TestApp : public tysoc::TApp
{
    ...
    static void testKeyCallback( int key, int action );
};
...
void TestApp::testKeyCallback( int key, int action )
{
    if ( key == GLFW_KEY_SPACE && action == GLFW_PRESS )
    {
        int _primitiveId = rand() % MAX_PRIMITIVES;

        auto _entity = new tysoc::TPrimitiveEntity( tysoc::TVec3(0,15,0), tysoc::TVec3(),
        ...
        TApp::INSTANCE->getWorld()->addEntity( _entity );
    }
}
